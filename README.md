...
The folder `framework` has a `src/main/java/io/approov/framework/volley/ApproovService.java` file. This provides a wrapper around the Approov SDK itself to make it easier to use with `Volley`. Show the terminal in Android Studio by clicking on the `Terminal` tab, typically located at the bottom of the screen:
...
* Look at the [`logcat`](https://developer.android.com/studio/command-line/logcat) output from the device. Information about any Approov token fetched or an error is output at the `INFO` level, e.g. `2020-02-10 13:55:55.774 10442-10705/io.approov.shapes I/ApproovHurlStack: Approov Token for https://shapes.approov.io/v2/shapes: {"did":"+uPpGUPeq8bOaPuh+apuGg==","exp":1581342999,"ip":"1.2.3.4","sip":"R-H_vE"}`. You can easily [check](https://approov.io/docs/latest/approov-usage-documentation/#loggable-tokens) the validity.
...
### API Domains
Remember you need to [add](https://approov.io/docs/latest/approov-usage-documentation/#adding-api-domains) all of the API domains that you wish to send Approov tokens for. If you make a volley request to a domain that has not been added then no Approov token header is added.
